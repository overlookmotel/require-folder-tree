...
function processFolder(path, options)
{
	...
	if (options.filesKey) resultFiles = result[options.filesKey] = result[options.filesKey] ? _.clone(result[options.filesKey]) : {};
	if (options.foldersKey) {
		if (result[options.foldersKey]) {
			resultFolders = result[options.foldersKey] = _.clone(result[options.foldersKey]);
			keepFolders = true;
		} else {
			resultFolders = result[options.foldersKey] = {};
		}
	}
	...
	folders.forEach(function(file) {
		...
		if (options.flatten) {
			for (var thisFile in resultFolder) {
				...
				if (options.flattenPrefix === true) {
					fileIndex = file + utils.capitalize(thisFile);
				} else if (options.flattenPrefix !== false) {
					fileIndex = file + options.flattenPrefix + thisFile;
				}
